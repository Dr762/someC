...
//number of rotations for computing subkeys
static const int DesRotations[16] = {
        1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1

};

//mapping for the permuted choice for subkeys
...
//mapping for initial permutaion of data blocks
...
//mapping for expansion permutation of data blocks
...
//tables for the s-box substs performed for data blocks
...
//mapping for p-box permutation data blocks
static const int DesPbox[32] = {
        16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10,
        2, 8, 24, 14, 32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4, 25
};

//mapping for the final permutation
...
//type whether to encipher or decipher data
typedef enum DesForD_ {
    encipher, decihper
} DesEorD;
...
static void permute(unsigned char *bits, const int *mapping, int n) {
    ...
    //permute buffer using n-entry mapping
...
static int des_main(const unsigned char *source, unsigned char *target,
    ...
    for (i = 0; i < 16; i++) {
        ...
        //permute and expand copy to the right block into 48bits
        ...
        if (direction == encipher) {
            ...
            //apply keys in decreasing order for deciphering
        ...
        for (j = 0; j < 8; j++) {
            //compute row and column into S-box tables
        ...
        //do p-box permutation for f
        permute(fblk, DesPbox, 32);

        //XOR left block and f
    ...
    //set target text to rejoined final right and left blocks
    memcpy(&target[0], rblk, 4);
    memcpy(&target[0], lblk, 4);
...
void des_decipher(const unsigned char *ciphertext, unsigned char *plaintext, const unsigned char *key){
    des_main(ciphertext, plaintext, key, decihper);
    return;
